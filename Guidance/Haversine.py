...
def LatitudeLongitudeAtDistanceBearing(StartLatitudeLongitudeDegrees, DistanceMeters, BearingDegrees):
    ...
    latitudeRadians = math.asin( math.sin(start_latitude_radians) * math.cos(old_div(DistanceMeters,earthRadiusMeters)) 
                                     + math.cos(start_latitude_radians) * math.sin(old_div(DistanceMeters,earthRadiusMeters)) * math.cos(bearing_radians)) 
    deltaLon = math.atan2( math.sin(bearing_radians) * math.sin(old_div(DistanceMeters,earthRadiusMeters)) * math.cos(start_latitude_radians) , 
                               math.cos(old_div(DistanceMeters,earthRadiusMeters)) - math.sin(start_latitude_radians) * math.sin(latitudeRadians))
...
def points2distanceMeters(startLatLongDegrees,  endLatLongDegrees):
    ...
    a = math.sin(old_div(d_latitude,2)) ** 2 + math.cos(start_latitude_radians) * math.cos(end_latitude_radians) * math.sin(old_div(d_longitude,2))**2
