...
class Atmosphere(object):
    ...
    def tas2cas(self, tas , altitude , temp='std', speed_units = 'm/s', alt_units = 'm'):
        ...
        cas = 1 + old_div(( mu * densityKgm3 * tas * tas), ( 2 *  pressurePascals))
        cas = math.pow(cas , 1.0 / mu) - 1.0
        

        cas = 1 + (old_div(pressurePascals, pressureMSLpascals)) * (cas)
        cas = math.pow(cas, mu) - 1.0
        cas = old_div(( 2 * pressureMSLpascals), (mu * densityMSLkgm3) * cas)
    ...
    def cas2tas(self, cas, altitudeMeters, speed_units = 'm/s', altitude_units = 'm'):
        ...
        tas = 1 + old_div(( mu * densityMSLkgm3 * cas * cas), ( 2 *  pressureMSLpascals))
        tas = math.pow(tas , 1.0 / mu) - 1.0
        

        tas = 1 + (old_div(pressureMSLpascals, pressurePascals)) * (tas)
        tas = math.pow(tas, mu) - 1.0
        tas = old_div(( 2 * pressurePascals), (mu * densityKgm3) * tas)
...
class Test_Main(unittest.TestCase):
    ...
    def test_mains(self):
        ...
        if 'FlightDynamics' in os.getcwdu():
            fileName = os.getcwdu() + os.path.sep + 'Home' +  os.path.sep + 'ResultsFiles' + os.path.sep + fileName
