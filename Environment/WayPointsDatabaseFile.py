...
from FlightProfile.src.xlrd import open_workbook
from FlightProfile.src.xlwt.Workbook import Workbook
...
def convertDegreeMinuteSecondToDecimal(DegreeMinuteSecond='43-40-51.00-N'):
    ...
    DecimalValue = 0.0
    coeff = 0.0
    assert isinstance(DegreeMinuteSecond, str) or isinstance(DegreeMinuteSecond, str)
...
class WayPointsDatabase(object):
    ...
    def __init__(self):
        ...
        if not('FlightProfile' in self.FilesFolder) and not('Environment' in self.FilesFolder):
    ...
    def read(self):
        ...
        for row in range(sheet.nrows): 
            ...
            if row == 0:
                ...
                if not(WayPointName in list(self.WayPointsDict.keys())):
                
                    wayPointDict = {}
                    for column in self.ColumnNames:
                        if column == 'Latitude' or column == 'Longitude':
                            ''' replace degree character '''
                            strLatLong = str(rowValues[self.ColumnNames[column]]).strip()
                            if u'°' in strLatLong:
                                strLatLong = str(strLatLong).replace(u'°','-')
    ...
    def getWayPoint(self, Name):
        ...
        if str(Name).upper() in list(self.WayPointsDict.keys()):
            ...
            messagebox.showerror("Error", "Waypoint not found")
            return None
    ...
    def getWayPoints(self):
        ''' Iterator '''
        for key in list(self.WayPointsDict.keys()):
    ...
    def insertWayPoint(self, wayPointName, Latitude, Longitude):
        ...
        if readStatus and not(self.hasWayPoint(wayPointName)):
            ...
            writableSheet.write(numRow , 3, str(Latitude) )
            writableSheet.write(numRow , 4, str(Longitude) )
